...
class Ramp(EventBehavior):
    def __init__(self, duration, ramp_type=RampType.LINEAR, start_value=0, end_value=1, func=None):
...
class Channel:
    def __init__(self, name, card_number, channel_number, reset, reset_value):
        self.name = name
...
class Analog_Channel(Channel):
    def __init__(self, name, card_number, channel_number, reset=False, reset_value=0, default_voltage_func=lambda a: a, max_voltage=10, min_voltage=-10):
...
class Digital_Channel(Channel):
    def __init__(self, name, card_number, channel_number, card_id, bitpos, reset=False, reset_value=0):
        super().__init__(name, card_number, channel_number, reset, reset_value)
...
class Event:
    def __init__(self, channel, behavior: EventBehavior, start_time=None, relative_time=None, reference_time="start", parent=None):
    ...
    def update_relative_time(self, new_relative_time, new_reference_time=None):
    ...
    def check_for_overlap(self, channel, behavior, start_time, end_time):
    ...
    def reassign_children(self, new_parent, delta):
        for child in self.children:
    ...
    def print_event_hierarchy(self, level=0, indent="    "):
...
class Sequence:
    ...
    def add_analog_channel(self, name, card_number, channel_number, reset=False, reset_value=0, default_voltage_func=lambda a: a, max_voltage=10, min_voltage=-10):
    ...
    def add_digital_channel(self, name, card_number, channel_number, card_id, bitpos, reset=False, reset_value=0):
        channel = Digital_Channel(name, card_number, channel_number, card_id, bitpos, reset, reset_value)
        self.channels.append(channel)
        return channel

    def add_event(self, channel_name, behavior: EventBehavior, start_time=None, relative_time=None, reference_time="start", parent_event=None):
        if start_time is not None and relative_time is not None:
            raise ValueError("Provide either start_time or relative_time, not both.")
    ...
    def find_event_by_time_and_channel(self, start_time, channel_name):
        channel = self.find_channel_by_name(channel_name)
        if channel is None:
            return None
    ...
    def add_event_in_middle(self, parent_start_time, parent_channel_name, child_events, relative_time, reference_time, behavior, channel_name):
    ...
    def update_event_absolute_time(self, start_time, channel_name, new_start_time):
    ...
    def update_event_relative_time(self, start_time, channel_name, new_relative_time, new_reference_time=None):
        event = self.find_event_by_time_and_channel(start_time, channel_name)
        if event is None:
            raise ValueError(f"Event not found for start_time {start_time} and channel {channel_name}")
    ...
    def delete_event(self, start_time, channel_name):
        event = self.find_event_by_time_and_channel(start_time, channel_name)
        if event is None:
            raise ValueError(f"Event not found for start_time {start_time} and channel {channel_name}")
    ...
    def print_event_tree(self, level=0, indent="    "):
    ...
    def plot(self, channels_to_plot=None, resolution=1, start_time=None, end_time=None):
    ...
    @staticmethod
    def from_json(json_input=None, file_name=None):
        ...
        def deserialize_event(event_data, parent=None):
